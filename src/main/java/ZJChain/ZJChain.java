    ...
    public boolean isChainValid() {
        ...
        for(int i = 1; i < blockChain.size(); i++) {
            ...
            for(int t = 0; i < curBlock.transactions.size(); t++) {
                ...
                //检查交易的签名
                try {
                    if(!currentTransaction.verifySignature(currentTransaction.wallet)) {
                ...
                if(currentTransaction.outputs.get(0).recipient != currentTransaction.strsrecipient) {
                    System.out.println("第" + t + "个交易的交易输出目的方错误！");
                    return false;
                }

                if(currentTransaction.outputs.get(1).recipient != currentTransaction.strsSender) {
